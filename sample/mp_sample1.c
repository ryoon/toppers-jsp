 ...
 * 
 *  @(#) $Id: mp_sample1.c,v 1.23 2007/03/27 08:55:29 hiro Exp $
 */
 ...
 *  '4' : 以降のコマンドは TASK4 に対して行う．
 *  '5' : 以降のコマンドは TASK5 に対して行う．
 ...
 *  'n' : タスクを chg_pid によりMASTER_PRCIDを割り付ける．
 *  'N' : タスクを chg_pid によりSUB1_PRCIDを割り付ける．
 *  'o' : 自タスクを chg_pid によりMASTER_PRCIDを割り付ける．
 *  'O' : 自タスクを chg_pid によりSUB1_PRCIDを割り付ける．
 ...
 *  'c' : 周期ハンドラを動作させる．
 *  'C' : 周期ハンドラを停止させる．
 *  'f' : (i)set_flg(TEST_FLG, data)を発行する．
 *  'z' : CPU例外を発生させる．
...
void task(VP_INT exinf)
{
	...
	ena_tex();
	while (1) {
		syscall(get_pid(TSK_SELF, &prcid));
		syslog(LOG_NOTICE, "task%d-%d is running (%03d).   %s",
		...
		case 'Y':
			...
			break;
		case 'o':
			syslog(LOG_INFO, "#%d#chg_pid(TSK_SELF, MASTER_PRCID)", tskno);
			syscall(chg_pid(TSK_SELF, MASTER_PRCID));
			break;
		case 'O':
			syslog(LOG_INFO, "#%d#chg_pid(TSK_SELF, SUB1_PRCID)", tskno);
			syscall(chg_pid(TSK_SELF, SUB1_PRCID));
			break;
#ifndef OMIT_VGET_TIM
		case 'V':
...
void cyclic_handler(VP_INT exinf)
{
	...
	syscall(iget_pid(&prcid));
	syslog(LOG_NOTICE, "cyclic_handler(%d)", prcid);
...
void main_task(VP_INT exinf)
{
	...
	do {
		...
		case 'r':
			...
			break;
		case 'n':
			syslog(LOG_INFO, "#chg_pid(%d, MASTER_PRCID) to %d", tskid, tskno);
			syscall(chg_pid(tskid, MASTER_PRCID));
			break;
		case 'N':
			syslog(LOG_INFO, "#chg_pid(%d, SUB1_PRCID) to %d", tskid, tskno);
			syscall(chg_pid(tskid, SUB1_PRCID));
			break;
		case 'c':
			sta_cyc(CYCHDR1);
			break;
