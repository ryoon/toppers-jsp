...
static const SPINIB spinib_table[TNUM_SIOP] = {
...
static SPCB	spcb_table[TNUM_SIOP];
...
rcv_count(SPCB *spcb)
{
	int count = spcb->rcv_write_ptr - spcb->rcv_read_ptr;

	if(count >= 0)
		return count;
	else
		return count + SERIAL_BUFSZ;
}
...
snd_count(SPCB *spcb)
{
	int count = spcb->snd_write_ptr - spcb->snd_read_ptr;
...
serial_initialize(VP_INT exinf)
{
	...
	sio_initialize();
	for (spcb = spcb_table, i = 0; i < TNUM_SIOP; spcb++, i++) {
...
serial_wri_chr(SPCB *spcb, char c)
{
	...
	_syscall(loc_cpu());
	if (snd_count(spcb) == 0U && !(spcb->snd_stopped)
		...
		buffer_full = ((snd_count(spcb)+1) == SERIAL_BUFSZ);
	}
	_syscall(unl_cpu());
	return(buffer_full);
}
...
serial_rea_chr(SPCB *spcb, char *c)
{
	...
	*c = spcb->rcv_buffer[spcb->rcv_read_ptr];
	...
	buffer_empty = (rcv_count(spcb) == 0U);

	/*
	 *  START を送信する．
	 */
	if (spcb->rcv_stopped && rcv_count(spcb) <= BUFCNT_START) {
...
serial_ref_por(ID portid, T_SERIAL_RPOR *pk_rpor)
{
	...
	pk_rpor->reacnt = rcv_count(spcb);
	pk_rpor->wricnt = snd_count(spcb);
	return(E_OK);
}
...
sio_ierdy_rcv(VP_INT exinf)
{
	...
	else if ((rcv_count(spcb)+1) >= SERIAL_BUFSZ) {
		/*
