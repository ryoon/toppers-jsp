 ...
 * 
 *  @(#) $Id: sys_defs.h 2246 2026-03-11 12:00:15Z roi $
 */
 ...
 *  ターゲットを識別するためのマクロの定義
 */
#define TOPPERS_CYCLONE_V		/* システム略称 */

/*
 *  起動メッセージのターゲットボード名
 */
#define	TARGET_NAME	"CORTEX-A9 - CYCLONE V"
...
/*
 *  IPIハンドラのディスパッチのバイパス設定
 */
#define USE_IPI_DIS_HANDER_BYPASS

/*
 *  TPCB取得にスレッドIDレジスタを使用する
 */
#define USE_LOCAL_MY_TPCB

/*
 *  UARTポートとしてFPGAUARTを使用する
 */
#define USE_FPGAUART
 ...
 *  スタック定義
 */
#define STACKTOP	0x01800000U	/* 非タスクコンテキスト用のスタックの初期値 */
#define PSTACKSIZE	8192U
...
typedef	UINT	INTNO;			/* 割込み番号（cpu_configで利用） */
