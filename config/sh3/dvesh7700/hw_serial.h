 ...
 * 
 *  @(#) $Id: hw_serial.h,v 1.3 2001/10/20 16:14:06 honda Exp $
 */
...
/*
 *  ターゲット依存シリアルI/Oモジュール（DVESH7700用）
 *  SCI1を使用
 */
 ...
 */
#define INHNO_SERIAL_IN	 SCI_RXI_INTEVT       
#define INHNO_SERIAL_OUT SCI_TXI_INTEVT	
...
{
    SCI.SCSCR = 0;          /* SCIを停止,内部Pクロックを使う */
    SCI.SCSMR = 0;          /* 8データ,1ストップビット,no parity */
    SCI.SCBRR = SCI_BPS_VALUE;        
    ...
    *RFCR = RFCR_CLEAR;
    ...
    SCI.SCSSR = 0x0060;         /* エラーフラグをクリア           */
    SCI.SCSCR = 0x70;           /* Start Tx Rx 受信割り込み許可   */
     ...
     */
    if (initflag[0] <= 0) {
        *IPRB=(*IPRB&0xff0f)|(SCIINTLVL<<4); /* 割り込みレベル設定 */
        define_int_plevel(SCI_RXI_INTEVT,SCIINTLVL);
        define_int_plevel(SCI_TXI_INTEVT,SCIINTLVL);
    }
...
{
	*(p->initflag) = -1;		/* 初期化フラグ設定           */
    SCI.SCSCR = 0x00;         /* シリアルコントローラの停止 */
    *IPRB=(*IPRB&0xff0f);       /* 割り込みマスククリア       */
}
...
{
    SCI.SCTDR=c;
    SCI.SCSSR &= ~(SCSSR_TDRE |SCSSR_TEND);
}
...
{
    if (!(p->sendflag)) {
        SCI.SCSCR |= SCSCR_TIE;   /* 送信割り込み要求を許可 */
        p->sendflag = 1;
    }
}
...
{
	if (p->sendflag) {
        SCI.SCSCR &= ~SCSCR_TIE; /* 送信割り込み要求を禁止 */
        p->sendflag = 0;
	}
}
