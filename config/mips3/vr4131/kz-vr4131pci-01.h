 ...
 */
/*
 *  VR4131 は起動時に CLKSEL(2:0) 端子を見て、パイプラインクロックPClockを決定す
 *  る。今回のターゲットボードのデフォルトは、
 *        CLKSEL(2:0) = 110
 ...
 *  デバイスの割込みマスクの定義
 *
 *  優先度 : (低) DSIU -> TClockカウンタ (高)
 */
/* MSYSINT1のBATINTRの設定については、ハードウェア編p224参照  */

/*
 *  DSIU割込みの割込みマスク（送受信共用）
...
#define IPM_DSIC	{ CORE_IPM_SCC, {ICU_IPM_SYSINT1_DSIU, ICU_IPM_SYSINT2_DSIU}}
