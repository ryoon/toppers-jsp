 ...
 *
 *  @(#) $Id: cpu_insn.h,v 1.2 2002/04/13 13:14:45 imai Exp $
 */
...
/*  以下の関数は、アセンブラ内で定義(cpu_support.src)  */
/*
...
disint(void)
{
    set_exr((UB)((current_exr() & ~0x07) |  MAX_IPM) );
}

/*
 *  すべての割込みを許可
 */
/*  割込みモード0の場合には、別の設定が必要  */
Inline void
enaint()
{
   set_exr((UB)(current_exr() & ~0x07));
 ...
 * ビットをクリアー
 */
void bitclr (UW addr, UB bit);


/*
 * ビットをセット
 */
void bitset (UW addr, UB bit);
