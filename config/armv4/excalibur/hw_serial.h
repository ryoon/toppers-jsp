 ...
 * 
 *  @(#) $Id: hw_serial.h,v 1.7 2003/12/11 00:44:27 honda Exp $
 */

/*
 *  シリアルI/Oデバイス（SIO）ドライバ（Excalibur-ARM用）
 */
...
sio_opn_por(ID siopid, VP_INT exinf)
{
    ...
    if(!openflag){
         ...
         * 割り込みレベル設定(ソフトウェア)
         */        
        define_inh_mask(INHNO_SIO,INT_MASK_UA);

        /*
         * 割り込みレベル設定(INTコントローラ)
         */
        /* 割込み優先度設定 */
        sil_wrw_mem((VP)INT_PRIORITY_UART,INTLV_UART);
        /* 割込みマスククリア割込み許可 */
        sil_wrw_mem((VP)INT_MASK_SET,INT_MASK_UA);
    }
...
sio_cls_por(SIOPCB *siopcb)
{
     ...
     */
    if (!uart_openflag) {
        sil_wrw_mem((VP)INT_MASK_CLEAR,INT_MASK_UA); /* 割込みマスクセット */
    }
}
