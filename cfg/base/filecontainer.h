...
public:
    ...
    virtual void                    attachModule(const std::string & filename) noexcept(false) = 0;
    virtual void                    loadContents(void * dest, address_t address, size_t size) noexcept(false) = 0;
    virtual address_t               getSymbolAddress(const std::string & symbol) noexcept(false) = 0;
    virtual std::string             getArchitecture(void) noexcept(false) = 0;

    virtual struct tagVariableInfo  getVariableInfo(const std::string & name) noexcept(false);
    virtual void                    attachInfo(const std::string & filename) noexcept(false);
#else
    ...
    virtual void                    attachInfo(const std::string & filename) throw(Exception);
        ...
        /* 登録されているコンテナの呼び出し */
#if __cplusplus >= 201703
    static inline FileContainer * getInstance(void) noexcept(false)
#else
...
protected:
    ...
    virtual ~TargetVariableBase(void) throw() {}
        ...
        /* 内容の取得 */
#if __cplusplus >= 201703
    void loadContent(void * dest, size_t dest_size) noexcept(false);
#else
...
public:
    ...
    {   return address != 0 && size != 0 && structure_size != 0;   }
...
#if __cplusplus >= 201703
    inline const T & operator * (void) noexcept(false)
#else
...
#if __cplusplus >= 201703
    inline T operator [] (int index) const noexcept(false)
#else
