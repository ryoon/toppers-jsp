...
namespace {
    ...
    protected:
        ...
        void loadSymbols(fstream & file)  noexcept(false);
        void loadDataContents(fstream & file) noexcept(false);

            //contentsへ1バイト書き込み
        void writeByte(address_t address, unsigned int) noexcept(false);
#else
    ...
    public:
        ...
        virtual ~FileContainerBinutilsImpl(void) throw();
            ...
            /* インタフェース部 */
#if __cplusplus >= 201703
        virtual void        attachModule(const string & filename) noexcept(false);
        virtual void        loadContents(void * dest, address_t address, size_t size) noexcept(false);
        virtual address_t   getSymbolAddress(const string & symbol) noexcept(false);
#else
    ...
    FileContainerBinutilsImpl::~FileContainerBinutilsImpl(void) throw()
    {
        ...
        /* ファイル名をカンマで二つに分ける */
#if __cplusplus >= 201703
    void splitFilename(const string & src, string & first, string & second) noexcept(false)
#else
...
#if __cplusplus >= 201703
    void openTextFile(fstream & file, const string & filename, FileContainerBinutilsImpl::interceptor_func_t * interceptor)noexcept(false)
#else
    ...
    const char * makeTemporaryFilename(void) throw()
    {
        ...
        /* バイナリをGNU-NMを使って変換する */
#if __cplusplus >= 201703
    void interceptWithGnuNM(fstream & file, const string & filename) noexcept(false)
#else
...
#if __cplusplus >= 201703
    void interceptWithGnuObjcopy(fstream & file, const string & filename) noexcept(false)
#else
    ...
    bool readGnuNmLine(fstream & file, FileContainer::address_t & address, string & attribute, string & symbolname) throw()
    {
		...
		{
			// jsp.symsがCR/LF行の場合、シンボル名が正しく設定されないので
			// src中のCRを削除する(ROI)
			int ssize = src.size();
			int crpos = src.find("\r");
			if(ssize == (crpos+1)){
				src.erase(crpos, 1);
			}
		}
        ...
        /* シンボルの読み込み */
#if __cplusplus >= 201703
    void FileContainerBinutilsImpl::loadSymbols(fstream & file) noexcept(false)
#else
...
#endif
    {
        ...
        /* contentsに1バイト書き込み */
#if __cplusplus >= 201703
    void FileContainerBinutilsImpl::writeByte(address_t address, unsigned int value) noexcept(false)
#else
    ...
    void trimString(string & src) throw()
    {
        ...
        */
#if __cplusplus >= 201703
    bool readRecord(fstream & file, string & dest) noexcept(false)
#else
    ...
    void chopRecord(string & src) throw()
    {
        ...
        /* プログラムデータの読み込み */
#if __cplusplus >= 201703
    void FileContainerBinutilsImpl::loadDataContents(fstream & file) noexcept(false)
#else
...
#if __cplusplus >= 201703
    void FileContainerBinutilsImpl::attachModule(const string & filename) noexcept(false)
#else
...
#endif
    {
        ...
        /* 格納している内容の取得 */
#if __cplusplus >= 201703
    void FileContainerBinutilsImpl::loadContents(void * _dest, address_t address, size_t size) noexcept(false)
#else
...
#endif
    {
        ...
        /* シンボルのアドレスの取得 */
#if __cplusplus >= 201703
    FileContainer::address_t FileContainerBinutilsImpl::getSymbolAddress(const string & symbol) noexcept(false)
#else
