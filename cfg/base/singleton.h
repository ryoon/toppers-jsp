...
public:
    ...
    public:
        ...
        void renewInstance(void) noexcept(noexcept(std::bad_alloc()))
#else
...
#endif
        {
            ...
            /* 全てのインスタンスを破棄 */
#if __cplusplus >= 201703
        void clearInstance(void) noexcept(noexcept(std::bad_alloc()))
#else
...
#endif
        {
            ...
            /* コンテキストの退避 */
#if __cplusplus >= 201703
        template<class T> void saveContext(void) noexcept(noexcept(std::bad_alloc()))
#else
...
#if __cplusplus >= 201703
    static T * createInstance(void) noexcept(noexcept(std::bad_alloc()))
#else
...
public:
        //インスタンスの取得
#if __cplusplus >= 201703
    inline static T * getInstance(void) noexcept(noexcept(std::bad_alloc()))
#else
    ...
    typedef T * Context;    //退避用
        ...
        //新しいインスタンスの生成
#if __cplusplus >= 201703
    static void renewInstance(void) noexcept(noexcept(std::bad_alloc()))
#else
