...
enum Token::tagTokenType Parser::getToken(Token & token, bool allow_space)
{
    ...
    token += static_cast<char>(ch);
	// ここでch == -1のケースがある、この場合EOSを設定する(ROI)
	if(ch == -1){
...
void Parser::getToken(const char * term) noexcept(false)
#else
...
#if __cplusplus >= 201703
void Parser::getToken(const char * first, const char * second, const char * third, const char * fourth) noexcept(false)
#else
