...
public:
    ...
    public:
        ...
        Part & operator << ( T src ) noexcept(false)
#else
...
#endif
        {
            ...
            //ストリームに書いた内容を受け取る
#if __cplusplus >= 201703
        std::string getContents(void) const noexcept(false);

            //ストリームの内容を直接設定する
        void setContents(std::string contents) noexcept(false);
#else
        ...
        void setContents(std::string contents) throw(Exception);
            ...
            //ストリーム名の参照
#if __cplusplus >= 201703
        inline const std::string & getName(void) const noexcept(false)
#else
...
public:
    ...
    {   return (this != 0) && !filename.empty();   }
        ...
        //ファイル名
#if __cplusplus >= 201703
    std::string setFilename(std::string filename) noexcept(false);
#else
...
#if __cplusplus >= 201703
        //ストリームの内容をファイルに出力
    bool serialize(void) noexcept(false);

        //部位の生成
    MultipartStream & createPart(std::string name, bool precedence = false) noexcept(false);

        //部位の選択
    MultipartStream & movePart(std::string name) noexcept(false);
#else
        //ストリームの内容をファイルに出力
    ...
    template <class T>
#if __cplusplus >= 201703
    inline MultipartStream & operator << ( T src ) noexcept(false)
#else
...
#endif
    {
        ...
        //部位の選択 (Synonym)
#if __cplusplus >= 201703
    inline MultipartStream & operator[] (std::string name) noexcept(false)
#else
