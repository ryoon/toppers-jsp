...
void FileContainer::attachInfo(const string & filename) noexcept(false)
#else
...
#endif
{
    ...
    /* 変数情報の取得 */
#if __cplusplus >= 201703
struct FileContainer::tagVariableInfo FileContainer::getVariableInfo(const string & name) noexcept(false)
#else
    ...
    : address(0), size(0), offset(0), structure_size(0), loaded(false)
{
    ...
    /* オブジェクト内容の取得 */
#if __cplusplus >= 201703
void TargetVariableBase::loadContent(void * dest, size_t dest_size) noexcept(false)
#else
...
public:
    ...
    string module;
#if __cplusplus >= 201703
    void attachModule(const std::string & _module) noexcept(false) 
#else
    ...
    size_t    sz;
#if __cplusplus >= 201703
    void loadContents(void * _dest, address_t _addr, size_t _sz) noexcept(false) 
#else
    ...
    string symbol;
#if __cplusplus >= 201703
    address_t getSymbolAddress(const std::string & _symbol) noexcept(false) 
#else
...
#if __cplusplus >= 201703
    std::string getArchitecture(void) noexcept(false)
#else
