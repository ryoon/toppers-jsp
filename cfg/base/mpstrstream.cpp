...
string MultipartStream::Part::getContents(void) const noexcept(false)
#else
...
#endif
{
    ...
    //ストリームの内容を直接設定する
#if __cplusplus >= 201703
void MultipartStream::Part::setContents(string contents) noexcept(false)
#else
...
MultipartStream::~MultipartStream(void) throw()
{
    ...
    //ファイル名の関連付け
#if __cplusplus >= 201703
string MultipartStream::setFilename(string _filename) noexcept(false)
#else
...
#if __cplusplus >= 201703
bool MultipartStream::serialize(void) noexcept(false)
#else
...
#endif
{
    ...
    //部位の作成
#if __cplusplus >= 201703
MultipartStream & MultipartStream::createPart(string name, bool precedence) noexcept(false)
#else
...
#endif
{
    ...
    //部位の選択
#if __cplusplus >= 201703
MultipartStream & MultipartStream::movePart(string name) noexcept(false)
#else
...
#endif
{
    ...
    if(!name.empty()) {
        ...
        //if(name.empty()) //手前のifの条件より、このifは常に真
