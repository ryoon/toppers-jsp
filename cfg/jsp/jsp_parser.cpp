...
namespace {
    ...
    string get_filename(string category, string _default) noexcept(false)
#else
    ...
    void serializeIdentifiers_defineStyle(KernelID * out, std::map<std::string, int> & identifier_pool)
    {
        ...
        //自動ID割付結果出力 (定数変数格納方式)
#if __cplusplus >= 201703
    void serializeIdentifiers_variableStyle(KernelID * id, std::map<std::string, int> & identifier) noexcept(false)
#else
...
#if __cplusplus >= 201703
    void serializeIdentifiers_blendedStyle(KernelID * id, std::map<std::string, int> & identifier) noexcept(false)
#else
...
#endif
    {
        ...
        //自動ID割付結果の出力
#if __cplusplus >= 201703
    void serializeIdentifiers(bool varid_style = false) noexcept(false)
#else
...
#if __cplusplus >= 201703
    void serializeConfiguration_headerPart(KernelCfg & out) noexcept(false)
#else
...
#ifdef JSP_FREERTOS
    createObjectDefinition(out, container[OBJECTTREE "/" TASK], HEADER|TNUM|SHORT_ID|BUFFER|DEFINITION|CONTROLBLOCK|INIT|PROTOTYPE|INITLIST,"tsk", "t", "void $(task)(VP_INT exinf);", "static __STK_UNIT __stack_$@[__STATIC_ALLOC(__TCOUNT_STK_UNIT($(stksz)))];",work.c_str());
#else	/* JSP_FREERTOS */
